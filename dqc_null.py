...
def check_null(db_name, table_name, columns):
    ...
    if(columns == '*'):
        ...
        query = f"SELECT number_columns, {str_columns} FROM {table_name} WHERE {' OR '.join([f'{col[0]} IS NULL' for col in tuple_columns])}"
    ...
    # If there are null values, send an email
    if null_rows:
